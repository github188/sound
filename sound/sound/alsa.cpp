	...
	unsigned int frequency, unsigned int buffer_time, unsigned int period_time)
{
	...
	{
		WW_LOG_ERROR("Playback open error: %s\n", snd_strerror(err));
#ifdef DEBUG
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Broken configuration for PLAYBACK: no configurations available: %s\n", snd_strerror(err));
#ifdef DEBUG
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Access type not available for  PLAYBACK: %s\n", snd_strerror(err));
#ifdef DEBUG
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Sample format not available for  PLAYBACK: %s\n", snd_strerror(err));
#ifdef DEBUG
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Unable to get format for  PLAYBACK: %s\n", snd_strerror(err));
#ifdef DEBUG
	...
	if (ft != fmt)
	{
		WW_LOG_ERROR("format doesn't match for  PLAYBACK(get %d):%s\n", ft, snd_strerror(err));
#ifdef DEBUG
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Channels count (%i) not available for  PLAYBACK: %s\n", channels, snd_strerror(err));
#ifdef DEBUG
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Rate %iHz not available for  PLAYBACK: %s\n", frequency, snd_strerror(err));
#ifdef DEBUG
	...
	if (rrate != frequency)
	{
		WW_LOG_ERROR("Rate doesn't match (requested %iHz, get %iHz)\n", frequency, err);
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Unable to set buffer time %i for  PLAYBACK: %s\n", buffer_time, snd_strerror(err));
#ifdef DEBUG
	...
	{
		WW_LOG_ERROR("buffer_time doesn't match (requested %d, get %i)\n", buffer_time, time_temp);
#ifdef DEBUG
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Unable to set period time %i for  PLAYBACK: %s\n", period_time, snd_strerror(err));
#ifdef DEBUG
	...
	{
		WW_LOG_ERROR("period_time doesn't match (requested %d, get %i)\n", period_time, time_temp);
#ifdef DEBUG
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Unable to set params(Init) for PLAYBACK: %s\n", snd_strerror(err));
#ifdef DEBUG
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Unable to get buffer size for  PLAYBACK: %s\n", snd_strerror(err));
#ifdef DEBUG
	...
	if (err < 0)
	{
		WW_LOG_ERROR("Unable to get period size for  PLAYBACK: %s\n", snd_strerror(err));
#ifdef DEBUG
...
void alsaPlayer::writeData(char * buf, int frames)
{
	...
	if ((p_temp - p_controlBuffer) <= controlBuffer_size) //未绕回
	{
		...
		memcpy(p_write, buf, frames*channels_*(snd_pcm_format_physical_width(format_) / 8));
		p_write = p_write + frames*channels_*(snd_pcm_format_physical_width(format_) / 8);
	}
	else//绕回
	{
		...
		if (no_extra_frames)
		{
			memcpy(p_write, buf, no_extra_frames*channels_*(snd_pcm_format_physical_width(format_) / 8));
			buf = buf + no_extra_frames*channels_*(snd_pcm_format_physical_width(format_) / 8);
		}
		p_write = p_controlBuffer;
		//处理绕回部分
		memcpy(p_write, buf, extra_frames*channels_*(snd_pcm_format_physical_width(format_) / 8));
...
int alsaPlayer::underrun_recovery(int err)
{
	...
	{    /* under-run */
		...
		if (err < 0)
			WW_LOG_ERROR("Can't recovery from overrun, prepare failed: %s\n", snd_strerror(err));
		return 0;
	}
	else if (err == -ESTRPIPE)
	{
		...
		if (err < 0)
		{
			err = snd_pcm_prepare(handle);
			if (err < 0)
				WW_LOG_ERROR("Can't recovery from suspend, prepare failed: %s\n", snd_strerror(err));
		}
		return 0;
	}
	return err;
...
int alsaTimer::Init(int intervals)//ms
{
	...
	{
		WW_LOG_ERROR("slave class is not set\n");
#ifdef DEBUG
	...
	if (!snd_timer_info_is_slave(info))
	{
		...
		{
			WW_LOG_ERROR("intervals does not match,get(%d)\n", t);
#ifdef DEBUG
...
#endif 
		WW_LOG_ERROR("pollfd malloc error\n");
		return -1;
	}
	while (1)
	{
...
#endif 
			WW_LOG_ERROR("snd_timer_poll_descriptors error: %s\n", snd_strerror(err));
			return err;
		}
